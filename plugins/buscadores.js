...
let img = 'https://wpbr.mx/blog/wp-content/uploads/2015/09/Nuevo-logo-de-google.jpg' || botimg.getRandom()
...
let translatedTitle = await translate(title, { to: lenguajeBot.lenguaje() || 'en', autoCorrect: true })
let translatedDesc = await translate(desc, { to: lenguajeBot.lenguaje() || 'en', autoCorrect: true })
...
text = m.quoted.text
} else return conn.reply(m.chat, lenguajeBot.smsOpenai1() + `\n*${usedPrefix + command}* ${lenguajeBot.smsOpenai2()}\n\n*${usedPrefix + command}* ${lenguajeBot.smsOpenai3()}` , m)
await conn.sendPresenceUpdate('composing', m.chat)
try {
let syms = `Juguemos un juego, tu apartir de ahora me responderas todas las preguntas como si fueras un Bot de WhatsApp el cual fue creado por Waifus Album, tu nombre sera Waifus Bot, recuerda actuar bien.`
...
case isCommand3:
if (!text) return conn.reply(m.chat, lenguajeBot.smsMalused2() + `\n*${usedPrefix + command} ${lenguajeBot.smsCreA()}*` , m) 
try{
await conn.sendPresenceUpdate('composing', m.chat)
let res = await fetch (`https://api.simsimi.net/v2/?text=${text}&lc=${lenguajeBot.lenguaje()}`)  
...
case isCommand4:
if (!text) return conn.reply(m.chat, lenguajeBot.smsGit1(usedPrefix, command), m)
await m.reply(lenguajeBot.smsGit2())
try{
let err = lenguajeBot.smsGit14()
...
${lenguajeBot.smsGit10()}
${followers || err}

${lenguajeBot.smsGit11()}
${following || err}

${lenguajeBot.smsGit12()}
${location || err}

${lenguajeBot.smsGit13()}
${type || err}`
await conn.sendFile(m.chat, thumb || botimg.getRandom(), 'githubstalk.jpg', cont, fkontak) 
} catch (e) {
reportError(e)}  
break   
        
case isCommand5:
if (!text) return m.reply(lenguajeBot.smsMalused2() + `*${usedPrefix + command}* waifus bot`)
try{
...
switch (v.type) {
case 'video': return `
>> *${lenguajeBot.smsytserh1()}*
» ${v.title || lenguajeBot.smsGit14()}

>> *${lenguajeBot.smsytserh2()}*
» ${v.url || lenguajeBot.smsGit14()}

>> *${lenguajeBot.smsytserh3()}*
» ${v.timestamp || lenguajeBot.smsGit14()}

>> *${lenguajeBot.smsytserh4()}*
» ${v.ago || lenguajeBot.smsGit14()}

>> *${lenguajeBot.smsytserh5()}*
» ${v.views || lenguajeBot.smsGit14()}`.trim()
}}).filter(v => v).join('')
...
break
case isCommand6:
if (!text) throw `${lenguajeBot.smsMalused2()}\n*${usedPrefix + command}* es Hello`
try {
let lang = args[0];
...
case isCommand7:
if (!text) throw `*${lenguajeBot['smsOpenai1']()} ${usedPrefix + command}* ${lenguajeBot.smsOpenai2()}\n\n*${usedPrefix + command}* ${lenguajeBot.smsOpenai3()}`
try {
